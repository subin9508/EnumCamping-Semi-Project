        ...
        <div class="footer-main-content">
            ...
            <main
                ...
                <div class="container">
                        ...
                        <form class="custom-form">
                            <table class="table">
                                ...
                                <tr>
                                    ...
                                    <td colspan="5"
                                        style="font-size: 18px;">${user.userName}</td>
                                </tr>
                                <tr>
                                    <td colspan="2">
                                    ...
                                    <td colspan="5"
                                        style="font-size: 18px;">${user.userEmail}</td>
                                </tr>
                                <tr>
                                    <td colspan="2">
                                        <div style="text-align: center;">
                                    ...
                                    <td colspan="5"
                                        style="font-size: 18px;">${user.userPhone}</td>
                                </tr>
                            </table>
                        </form>
                    </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                <th
                                    style="font-size: 21px; font-weight: bold; background-color: #7C9C63; color: white;">선택
                                    구역</th>
                                <th
                                    style="font-size: 21px; font-weight: bold; background-color: #7C9C63; color: white;">가격</th>
                            </tr>
                        </thead>
                        <tbody id="areaOrder">
                            <c:forEach items="${reservationDetails}"
                                var="item">
                                <c:choose>
                                    <c:when
                                        test="${item.itemId ge 1 and item.itemId le 4}">
                                        <c:set var="area" value="캠핑존 A-1" />
                                        <c:set var="price"
                                            value="${item.itemAmount}" />
                                    </c:when>
                                    <c:when
                                        test="${item.itemId ge 5 and item.itemId le 8}">
                                        <c:set var="area" value="캠핑존 A-2" />
                                        <c:set var="price"
                                            value="${item.itemAmount}" />
                                    </c:when>
                                    <c:when
                                        test="${item.itemId ge 9 and item.itemId le 12}">
                                        <c:set var="area" value="캠핑존 B-1" />
                                        <c:set var="price"
                                            value="${item.itemAmount}" />
                                    </c:when>
                                    <c:when
                                        test="${item.itemId ge 13 and item.itemId le 16}">
                                        <c:set var="area" value="캠핑존 B-2" />
                                        <c:set var="price"
                                            value="${item.itemAmount}" />
                                    </c:when>
                                    <c:when
                                        test="${item.itemId ge 17 and item.itemId le 20}">
                            ...
                            <tr>
                                <td style="font-size: 18px;">${area}</td>
                                <td style="font-size: 18px;">${price}원</td>
                            </tr>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                <th
                                    style="font-size: 21px; font-weight: bold; background-color: #7C9C63; color: white;">물품
                                    이름</th>
                                <th
                                    style="font-size: 21px; font-weight: bold; background-color: #7C9C63; color: white;">물품
                                    사진</th>
                                <th
                                    style="font-size: 21px; font-weight: bold; background-color: #7C9C63; color: white;">수량</th>
                                <th
                                    style="font-size: 21px; font-weight: bold; background-color: #7C9C63; color: white;">총가격</th>
                            </tr>
                        </thead>
                        <tbody id="orderDetails">
                            ...
                            <c:if test="${!hasItems}">
                                <tr>
                                    <td colspan="4"
                                        style="text-align: center; font-size: 18px;">선택하신
                                        구매/대여물품이 없습니다.</td>
                                </tr>
                            </c:if>
                        </tbody>
                    ...
                    <div class="mt-2">
                        <label for="requirement" class="form-label"
                            style="font-size: 18px;">요청사항</label>
                        <textarea id="requirement" class="form-control"
                            rows="5" style="font-size: 18px;" readonly><c:set
                                var="requirementValue"
                                value="${empty reservationMaster.requirement ? '요청없음' : reservationMaster.requirement}" />
                                ${requirementValue}
                                </textarea>
                    </div>
                    <br /> <br />
                ...
                <div
                    ...
                    <button id="btnPayment" class="btn btn-payment"
    ...
    <script>
					...
					function checkAgreements() {
						...
						if (agreeCollect && agreeProvide) {
							...
							document.getElementById('btnPayment').setAttribute(
									'disabled', 'disabled');
						}
					}
					...
					function calculateTotalAmount() {
						...
						itemAmountElements.forEach(function(element) {
							totalAmount += parseInt(element.textContent
									.replace('원', ''), 10);
						});
